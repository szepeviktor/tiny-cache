...
function get_the_content_cached( $more_link_text = null, $strip_teaser = false ) {
    ...
    // @ransient
    $cached = get_transient( 'get_the_content:' . $post_id )
    $found = ( false === $cached );
    ...
    if ( true === $save_to_cache ) {
        ...
        // @transient
        set_transient( 'get_the_content:' . $post_id, $content . $message, DAY_IN_SECONDS )
    }
...
function tiny_cache_save_the_content( $content ) {
    ...
    if ( $post_id ) {
        ...
        $message = sprintf( $message_tpl, esc_html( $timestamp ) );
        // @transient
        set_transient( 'the_content:' . $post_id, $content . $message, DAY_IN_SECONDS )
    }
...
function tiny_cache_delete_the_content( $post_id ) {
    ...
    // @transient
    delete_transient( 'the_content:' . $post_id );
}
